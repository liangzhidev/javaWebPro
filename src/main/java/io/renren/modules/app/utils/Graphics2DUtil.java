...
//        num = 90;
        ...
        g.drawString("★", CENTERX-(num/2), CENTERY+(num/3)+10);
...
//        g.drawString(centerName, CENTERX -(num1), CENTERY +(30+50));
        ...
        //添加日期
//        g.setFont(new Font("宋体", Font.LAYOUT_LEFT_TO_RIGHT, 20));// 写入日期
//        g.drawString(year, CENTERX -(66), CENTERY +(30+78));
        ...
        if (odd) {
            g.setFont(f);
            String cha=messages[first]+"";
            g.drawString(cha, (float)(x0 - char_interval/2), (float)y0);
            ...
            for (int i=first+1;i<ilength;i++)
            {
                ...
                g.setFont(f2);
                cha=messages[i]+"";
                g.drawString(cha, (float)(x0 + ax - char_interval/2* Math.cos(aa)), (float)(y0 + ay - char_interval/2* Math.sin(aa)));
            }
            //中心点的左边
    ...
    public static void main(String[] args) {
        ...
        BufferedImage image = startGraphics2D("济南良知科技有限公司","电子签章",year);
        
        String filePath = "";
        try {
            filePath = "F:\\123\\"+new Date().getTime()+".png";
